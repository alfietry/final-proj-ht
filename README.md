...
This is a tiny demo that compares OpenAI's GPT-4 and a local Ollama Llama-3 model on a one-sample t-test task.
...
- Ollama must be running locally at `http://localhost:11434` with model name `llama3` to use the Ollama step. If not available, that step is skipped.
